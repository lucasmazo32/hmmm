...
export default function WelcomeContainer({ type, handleUser, handleClient }) {
  return (
    <div className={`welcome-${type}`}>
      <h2>{`Are you a ${type}?`}</h2>
      ...
      { type === 'user'
        ? <button onClick={handleUser} id="login-user" type="button" className="btn btn-welcome">Log in</button>
        : <button onClick={handleClient} id="login-client" type="button" className="btn btn-welcome">Log in</button>}
    </div>
  );
}
